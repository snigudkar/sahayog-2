...
function Navbar() {
  const [mobileDrawerOpen, setMobileDrawerOpen] = useState(false);
  const { isLoggedIn, handleLogout, user, userRole } = useAuth();

  useEffect(() => {
    console.log('Navbar useEffect - isLoggedIn:', isLoggedIn, 'user:', user, 'userRole:', userRole);
              ...
              {isLoggedIn ? (
                <>
                  <span className='text-sm text-neutral-500'>
                    Welcome, {userRole === 'admin' ? 'Admin' : 'User'} {user?.email}
                  </span>
